...
function print_html_nav($count_items,$page_number,$serch_fields=""){
    ...
    <input type="hidden" id="page_number" name="page_number" value="<?php if (isset($_POST['page_number'])){  echo esc_js(esc_html(stripslashes($_POST['page_number'])));} else { echo '1';} ?>"  />
    
    <input type="hidden" id="serch_or_not" name="serch_or_not" value="<?php if(isset($_POST["serch_or_not"])){ echo esc_js(esc_html(stripslashes($_POST["serch_or_not"])));}  ?>"    />
<?php
